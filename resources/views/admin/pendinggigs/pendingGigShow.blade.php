        ...
        <div class="wt-freelancerholder wt-rcvproposalholder">
            ...
            <div class="wt-managejobcontent">
                <div class="wt-userlistinghold wt-featured wt-proposalitem">
                    ...
                    <div>
                        <a href="{{ route('pendingGigs', $pendingGigShow->id) }}" class="wt-btn"
                           style="margin-left: 70%"><i
                                class="fa fa-arrow-left"></i> Back</a>
                    </div>
                    <div>
                        @if ($pendingGigShow->status == '0')
                            <form action="{{ route('pendingGigsApproved', $pendingGigShow->id) }}">
                                @csrf
                                @method('PUT')
                            ...
                            <form action="{{ route('gigCanceledReason', $pendingGigShow->id) }}">
                                @csrf
                                @method('PUT')
                                <button type="submit" class="btn btn-danger btn-sm p-2 m-1"
                                        onclick="return confirm('Are you sure to Reject this Gig?')"><i
                                        class="fa fa-times mr-2"></i>Canceled
                                </button>
                            </form>
                        @endif
                        @if ($pendingGigShow->status == '1' || $pendingGigShow->status == '2')
                            <form action="{{ route('pendingGigsCancelled', $pendingGig->id) }}" method="PUT">
                                @csrf
