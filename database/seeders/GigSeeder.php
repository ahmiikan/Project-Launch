...
class GigSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $gg = Freelancer::pluck('id');

        $gig_categories = GigCategory::pluck('id')->toArray();

        for ($i = 0; $i < count($gigTitle); $i++) {
            {
                $fk = $gg[$i];

                $gig[$i] = Gig::create([
                    'image' => '20220909175117.jpg',
                    'amount' => 50,
                    'gig_commission' => 10,
                    'gig_amount_after_commission' => 40,
                    'duration' => 10,
                    'status' => (bool)random_int(0, 1),
                    'title' => $gigTitle[$i],
                    'description' => $gigDescription[$i],
                    'freelancer_id' => $fk,
                    'category_id' => $gig_categories[array_rand($gig_categories)],
                ]);
